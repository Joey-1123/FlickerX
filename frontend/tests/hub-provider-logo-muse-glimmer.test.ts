...
test("the FlickerX re-upload of Muse Glimmer shows the Meta mark", () => {
  // The row the Model hub actually lists. Owner is testorg, so the org rule
  // never fires and the repo name has to carry it.
  assert.equal(
    resolveOwnerProviderLogo("flickerx", "Muse-Glimmer-30B-GGUF")?.logoPath,
    META,
  );
});
...
test("every Muse Glimmer variant rides the same stem", () => {
  for (const repo of [
    ...
    "Muse-Glimmer-30B-testorg-bnb-4bit",
