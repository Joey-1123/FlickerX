...
test("the catalog still wins over the tag", () => {
  assert.equal(routeFor("testorg/whisper-small", null), "stt");
  assert.equal(routeFor("testorg/Qwen3-ASR-0.6B-GGUF", null), "stt");
  assert.equal(routeFor("testorg/orpheus-3b-0.1-ft", null), "tts");
});
