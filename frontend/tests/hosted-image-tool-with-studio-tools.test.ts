...
// that regressed is structural though: which names the Studio-tools branch puts
...
// this PR an OpenAI or Gemini connection never took the Studio branch, so the
// hosted branch below always carried them; now that Search, Code, MCP or a
// project's automatic RAG selects the Studio branch instead, a list of purely
// local names would leave a lit Images pill out of the request entirely.
test("the Studio-tools branch still asks for the hosted tools Studio cannot run", () => {
...
// The other half of the same rule: Studio runs search itself on this path, so
...
test("it does not ask the provider for the tools Studio is running locally", () => {
  const branch = studioToolsBranch();
