...
// Each greeting carries its matching picture so a line always shows the
...
function buildWelcome(hour: number, name: string): Welcome {
  const g = (text: string, avatar: string): Welcome => ({ text, avatar });
...
const ThreadWelcome: FC<{
  ...
  const showGreetingAvatar = useUserProfileStore((s) => s.showGreetingAvatar);
  ...
  const currentEmojiSrc = `flickerx-avatars/${welcome.avatar}`;
    ...
    <div className="aui-thread-welcome-root mx-auto my-auto flex w-full max-w-(--thread-max-width) grow flex-col">
      <div className="aui-thread-welcome-center flex w-full grow flex-col items-center justify-start pt-[27.5dvh]">
        <div className="aui-thread-welcome-message flex w-full flex-col justify-center gap-9 px-4">
          {/* Center the greeting (avatar + title) over the composer. */}
          <div className="flex flex-row items-center justify-center gap-[15px]">
            {/* Temporary chat keeps the title on its own, no mascot. */}
            {showGreetingAvatar && !incognito && (
              <MascotImg
                src={currentEmojiSrc}
                className="size-[44px] -translate-y-[2px]"
              />
            )}
            <h1 className="aui-thread-welcome-message-inner flickerx-welcome-title fade-in slide-in-from-bottom-1 animate-in text-3xl tracking-[-0.02em] duration-200">
...
const Composer: FC<{
  ...
  const composerContent = (
    <>
      ...
      <div
        className="flickerx-composer-line"
      ...
      >
        <div
          ref={pillRowRef}
          className="flickerx-composer-left"
        ...
        {isDictating ? (
          ...
          <>
            <ComposerPrimitive.Input
              ...
              ref={inputRef}
              className="aui-composer-input flickerx-composer-input"
              minRows={1}
              maxRows={12}
              autoFocus={!disabled}
              disabled={disabled}
  ...
  return (
    ...
    >
      ...
      {isTauri ? (
        ...
        <div className="aui-composer-attachment-dropzone flickerx-composer-surface relative z-10">
          {composerContent}
        </div>
      ) : (
        <ComposerPrimitive.AttachmentDropzone className="group/dropzone aui-composer-attachment-dropzone flickerx-composer-surface relative z-10">
          {composerContent}
...
// a WSL-hosted FlickerX (issue #5546), `compositionend` never fires after the
...
const ReasoningToggle: FC<{ side?: "top" | "bottom" }> = ({
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild={true}>
          <button
            type="button"
            disabled={disabled}
            className="flickerx-thinking-pill"
          ...
          >
            <ThinkIcon />
            {activeLook ? (
              <span className="flickerx-thinking-label">
                {isEffort ? `Thinking · ${effortLabel}` : "Thinking"}
              </span>
            ) : null}
            <ArrowDownStandardIcon className="flickerx-thinking-caret size-[15px]" />
          </button>
        </DropdownMenuTrigger>
        <DropdownMenuContent
          side={side}
          ...
          className="flickerx-plus-menu flickerx-thinking-menu min-w-0 w-[176px]"
        >
          {isEffort ? (
            <>
  ...
  return (
    ...
    >
      ...
      {activeLook ? (
        <span className="flickerx-thinking-label">Thinking</span>
      ) : null}
    </button>
  );
};
  ...
  const plusMenuNodes: Record<PlusMenuItemId, ReactNode> = {
    ...
    savedPrompts: (
      <DropdownMenuSub>
        ...
        <DropdownMenuSubContent
          collisionPadding={16}
          className="flickerx-plus-menu w-[208px]"
        >
          {recentPrompts.map((p) => (
            <DropdownMenuItem
              key={p.id}
    ...
    projects: (
      <DropdownMenuSub>
        ...
        <DropdownMenuSubContent className="flickerx-plus-menu w-[232px]">
    ...
    >
      <DropdownMenuTrigger asChild={true}>
        <button
          ...
          className="flickerx-composer-plus"
      ...
      <DropdownMenuContent
        ...
        sideOffset={0}
        avoidCollisions={true}
        className="flickerx-plus-menu w-[244px]"
      ...
      >
        ...
        {overflowPlusItems.length > 0 ? (
          <DropdownMenuSub>
            ...
            <DropdownMenuSubContent className="flickerx-plus-menu w-[248px]">
  ...
  return (
    <div className="aui-composer-action-wrapper flex shrink-0 items-center gap-1.5">
      ...
      <ComposerPrimitive.If dictation={false}>
        ...
        >
          {/* size-[22px] is the fallback; flickerx-dictate-icon sets the size. */}
          <MicIcon className="flickerx-dictate-icon size-[22px]" />
        </TooltipIconButton>
        ...
        <ComposerPrimitive.Send asChild={true}>
          ...
          >
            {pendingSend ? (
              ...
              <ArrowUpIcon className="flickerx-send-icon aui-composer-send-icon size-[21px] stroke-2" />
            )}
          </TooltipIconButton>
