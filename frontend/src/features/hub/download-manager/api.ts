...
function parseErrorText(status: number, body: unknown): string {
  if (body && typeof body === "object") {
    ...
    if (status === 405) {
      return `${formatted || "Method Not Allowed"} - the FlickerX backend did not accept this API method. Restart FlickerX so the frontend and backend are on the same build.`;
    }
  ...
  if (status === 405) {
    return "Method Not Allowed - the FlickerX backend did not accept this API method. Restart FlickerX so the frontend and backend are on the same build.";
  }
