...
import {
  type FlickerXSupport,
  type FlickerXSupportStatus,
  classifyFlickerXSupport,
  excludedFormatTagsForDevice,
} from "../lib/flickerx-support";
...
export { classifyFlickerXSupport };
export type { FlickerXSupport, FlickerXSupportStatus };
...
function makeMapModel(
  ...
  return (raw: unknown): HfModelResult | null => {
    ...
    // A repo cross-tagged "gguf" that is actually a diffusers pipeline (e.g. a FlickerX *-bnb-4bit image model) ships no .gguf files, so the variant
...
/** FlickerX results pulled up-front before yielding general results. */
const UNFLICKERX_PREFETCH = 20;
/** With a typed query, float only a few flickerx results before the general listing. */
const UNFLICKERX_QUERY_PREFETCH = 3;
/** With a publisher query, fewer flickerx results before the pinned publisher model. */
const UNFLICKERX_PINNED_PREFETCH = 4;
...
/** Merged generator yielding flickerx-owned models first, then deduped general results. */
async function* mergedModelIterator(
  ...
  const flickerxIter = mergeTaskIterators(
    tasks,
    (task, taskSignal) =>
      listModels({
  ...
  const limit = pinnedId
    ? UNFLICKERX_PINNED_PREFETCH
    : query.trim()
      ? UNFLICKERX_QUERY_PREFETCH
      : UNFLICKERX_PREFETCH;

  // Phase 1: flickerx models first
  const seen = new Set<string>();
  let count = 0;
  for await (const model of flickerxIter) {
...
/** Yields priority models (fetched individually for full metadata), then the flickerx listing. */
...
// (in sort order), then the rest deduped, floating flickerx to the top under any sort.
async function* channelFlickerXFirstIterator(
  ...
  for await (const model of flickerxIter) {
    ...
    yield model;
    if (++count >= UNFLICKERX_CHANNEL_PREFETCH) break;
  }
...
export function useHubModelSearch(
  ...
  const flickerxOnly = ownerScope === "flickerx";
  ...
  const { isPublisherQuery, searchQuery, pinnedId, trimmed } = useMemo(() => {
    ...
    const is = !!m && m[1].toLowerCase() !== "flickerx";
    return {
      isPublisherQuery: is,
  ...
  const createIter = useCallback(
    (signal: AbortSignal) => {
      // Channel scoping bypasses the flickerx-merge iterator: a hard owner/tag filter shows that slice.
      if (channelOwner || channelTagsKey || channelQuery) {
        ...
        // Ownerless tag/format channels (e.g. GGUF filter): float flickerx-owned models first.
        if (pinFlickerXFirst && channelTagsKey && !channelOwner) {
          return channelFlickerXFirstIterator(
      ...
      if (!trimmed) {
        // No query: show priority models first (with full metadata), then general flickerx listing
      ...
      // metadata. For an "owner/repo" query, strip the org prefix so flickerx variants surface, then
      // pin the original publisher model. FlickerX-owned queries are left as-is.
  ...
  // Secondary sort only with no user query (the merged iterator already floats flickerx results)
  // and outside channel scoping.
  //
  // STABLE-APPEND CONTRACT: when a later page lands, keep the sorted prefix verbatim and append
  // only the new tail, else a late flickerx/* repo jumps earlier and bumps the viewport. Sort only
  ...
  // Owner-scoped channels return one owner, so re-sorting is a no-op; tag/format channels float flickerx.
  const sortingDisabled =
    !pinFlickerXFirst || isPublisherQuery || trimmed || Boolean(channelOwner);
