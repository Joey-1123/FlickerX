...
const CHANNEL_IDS: readonly ChannelId[] = ["flickerx-trending"];
...
function channelRecord<T>(value: T): Record<ChannelId, T> {
  return {
    "flickerx-trending": value,
    "flickerx-latest": value,
    "flickerx-safetensors": value,
  };
}
...
export function useHubFeed(opts: {
  ...
  return {
    trending: buildSection("flickerx-trending"),
    refetch,
  };
}
