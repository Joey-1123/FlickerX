...
// Inference-only quant formats FlickerX cannot fine-tune. Matched on the repo
...
// Quant methods FlickerX can fine-tune: full precision (none) or bitsandbytes.
const FINETUNABLE_QUANT = new Set(["bitsandbytes", "bnb", "bnb_4bit"]);

export function isFlickerxFinetunable(result: {
  id: string;
  quantMethod?: string;
}): boolean {
