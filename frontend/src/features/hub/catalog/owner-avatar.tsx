...
export function OwnerAvatar({
  ...
  if (isFlickerXOwner(owner) && !remote) {
    return (
      <ProviderLogoTile
        provider={UNFLICKERX_OWNER_LOGO}
        size={size}
        className={className}
      />
    );
...
export function useAvatarImageUrl(
  ...
  const isFlickerxOwner = isFlickerXOwner(owner);
  const remoteUrl = useHfOwnerAvatar(
    owner.trim() || "?",
    provider == null && !isFlickerxOwner,
  );
  if (provider) {
    return provider.treatment === "original" ? provider.logoPath : null;
  }
  return isFlickerxOwner ? UNFLICKERX_OWNER_LOGO.logoPath : remoteUrl;
}
