...
export const ModelInspector = memo(function ModelInspector({
  ...
  const flickerxSupport = useMemo<FlickerXSupport>(() => {
    return classifyFlickerXSupport({
  ...
  const paramsLabel = model.totalParams
    ...
    : "N/A";
  const flickerxSupported = flickerxSupport.status !== "unsupported";
  ...
  const canRunModel =
    ...
    !isEmbeddingOnly &&
    (model.isGguf || (!chatOnly && flickerxSupported));
  const canTrainModel =
    !isDataset &&
        ...
        <InspectorDownloadSlot>
          {model.isLocal && !hasActiveHubDownload ? (
            <LocalOnDeviceCard
              ...
              unsupportedReason={
                flickerxSupport.status === "unsupported" && !flickerxSupport.supportedIn
                  ? (flickerxSupport.reason ?? "Unsupported format")
                  : null
              }
              onLoad={onLoadLocal}
