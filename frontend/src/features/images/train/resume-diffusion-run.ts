 ...
 *
 * The same shape as the LLM tab's `resumeTrainingRun`: the run's own config is the source of
 * truth (so the resumed job trains with exactly the hyperparameters it was stopped with), with
 * `resume_from_checkpoint` pointed at the run's output directory. The backend picks the newest
 * complete `checkpoint-<N>` bundle inside it and continues at N+1, up to the SAME `train_steps`
 * target the original run had -- resuming a stop at 11 of 500 trains steps 12..500, it does not
 * queue another 500.
 *
 * Pure and throwing: the caller owns the toast. The thrown message is the backend's
 * `resume_blocked_reason` whenever it has one, since the server explains a refusal precisely and
 * a generic message here would contradict it.
 */
export function buildDiffusionResumePayload(
  ...
  if (!config.base_model || !config.data_dir || !config.output_dir) {
    // A record from a much older backend (or a hand-edited one) cannot be replayed safely: a
    // missing path would resolve somewhere else entirely and train the wrong thing.
    throw new Error(
