...
// Export filename, e.g. FlickerX_video_20260624-143005_123.mp4.
...
function exportFilename(video: GalleryVideo, format: VideoExportFormat = "mp4"): string {
  ...
  return `FlickerX_video_${stamp}_${video.seed}.${format}`;
}
...
function VideoGenerator({ active = true }: { active?: boolean }) {
  ...
  const handleModelSelect = useCallback(
    (id: string, meta: ModelSelectorChangeMeta) => {
      ...
      // Otherwise treat it as a full diffusers repo. The backend gates loads to FlickerX/* repos, the family bases, or on-device paths.
      if (meta.source !== "local" && !id.toLowerCase().startsWith("unsloth/")) {
        toast.error("Only FlickerX or on-device video models can be loaded here");
        abandonPick();
        return;
      }
