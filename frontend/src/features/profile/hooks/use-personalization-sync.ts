...
function sameProfile(a: ProfileSnapshot, b: ProfileSnapshot): boolean {
  return (
    ...
    a.showGreetingAvatar === b.showGreetingAvatar
  );
}
...
function profileSnapshot(): ProfileSnapshot {
  ...
  return {
    ...
    showGreetingAvatar: s.showGreetingAvatar,
  };
}
...
export function usePersonalizationSync(enabled: boolean): void {
    ...
    void (async () => {
      try {
        ...
        if (remote.saved) {
          ...
          const localGreeting =
            useUserProfileStore.getState().showGreetingAvatar;
          const remoteGreeting = remote.profile.showGreetingAvatar !== false;
          const keepLocalGreeting =
            remote.greetingAvatarSaved === false && localGreeting === false;
          const nextProfile: ProfileSnapshot = {
            ...
            showGreetingAvatar: keepLocalGreeting
              ? localGreeting
              : remoteGreeting,
          };
          ...
          lastSavedRef.current = serialized({
            ...payload(
              { ...nextProfile, showGreetingAvatar: remoteGreeting },
              nextTheme,
              remotePalette,
              storedRemoteCustomization,
              nextLanguage,
  ...
  useEffect(() => {
    ...
    const current = payload(
      { displayName, nickname, avatarDataUrl, avatarShape, showGreetingAvatar },
      theme,
      palette,
      customization,
      language,
