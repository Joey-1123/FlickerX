...
const SUPPORTED_AGENTS: AgentDetails[] = [
  {
    id: "claude",
    name: "Claude Code",
    docsUrl: "https://flickerx.ai/docs/basics/claude-code",
    logo: "anthropic",
  },
  {
    id: "codex",
    name: "OpenAI Codex",
    docsUrl: "https://flickerx.ai/docs/basics/codex",
    logo: "openai",
  },
  {
    id: "hermes",
    name: "Hermes Agent",
    docsUrl: "https://flickerx.ai/docs/integrations/hermes-agent",
    icon: "hermes.svg",
  ...
  },
  {
    id: "openclaw",
    name: "OpenClaw",
    docsUrl: "https://flickerx.ai/docs/integrations/openclaw",
    icon: "openclaw.svg",
  },
  {
    id: "opencode",
    name: "OpenCode",
    docsUrl: "https://flickerx.ai/docs/integrations/opencode",
...
const REMOTE_CMD_UNIX = `export UNFLICKERX_STUDIO_URL=https://studio.example.com
export UNFLICKERX_API_KEY=sk-flickerx-...
flickerx start claude`;

// PowerShell uses $env: assignments; export is POSIX-only.
const REMOTE_CMD_WINDOWS = `$env:UNFLICKERX_STUDIO_URL = "https://studio.example.com"
$env:UNFLICKERX_API_KEY = "sk-flickerx-..."
flickerx start claude`;
...
export function AgentsTab() {
  ...
  // snapshot path; `flickerx start` now also matches a path by the basename
  ...
  // A bare `flickerx start` attaches to whatever is loaded, which is the only way
  ...
  useEffect(() => {
    ...
    // root may not cover it), but `flickerx start --model <path>` is valid, so
  ...
  // No GGUF warning for `codex` (flickerx_cli's _require_gguf_for_codex): the
    ...
    <div className="flex min-w-0 max-w-full flex-col gap-8">
      ...
      >
        {/* The chip is the docs entry point, so no separate link is needed.
            No aria-label: it would replace the visible "flickerx start" as the
