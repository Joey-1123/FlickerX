...
// Build the `flickerx start <agent>` command for the API-keys panel.
// `flickerx start` reads UNFLICKERX_STUDIO_URL (default 127.0.0.1:8888) and only
...
// The bare `flickerx start` probes exactly http://127.0.0.1:8888, so only that literal
// host earns the bare command. `localhost` can resolve to ::1 (and `::1` is never
// probed), so both keep an explicit UNFLICKERX_STUDIO_URL -- harmless when they alias
...
export function buildAgentCommand(
  ...
  const bare = `flickerx start ${agent}`;
  ...
  // loopback on the same port must keep its explicit UNFLICKERX_STUDIO_URL.
  ...
  return os === "windows"
    ? `$env:UNFLICKERX_STUDIO_URL="${studioUrl}"; ${cmd}`
    : `UNFLICKERX_STUDIO_URL=${studioUrl} ${cmd}`;
}
