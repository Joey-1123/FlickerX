...
export function EmbeddingModelCombobox({
  ...
  const { results, isLoading } = useHubModelSearch(debouncedQuery, {
    ...
    excludeGguf: true,
    enabled: !disabled,
    // Curated flickerx listing when empty (the global top-downloads page holds
    // no flickerx mirrors to float); a typed query searches the whole Hub.
    ownerScope: debouncedQuery.trim() ? "all" : "flickerx",
  });
