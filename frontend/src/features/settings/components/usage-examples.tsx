...
const DOC_LINKS = [
  { label: "Claude Code", href: "https://flickerx.ai/docs/basics/claude-code" },
  { label: "Codex", href: "https://flickerx.ai/docs/basics/codex" },
  { label: "OpenClaw", href: "https://flickerx.ai/docs/integrations/openclaw" },
  { label: "OpenCode", href: "https://flickerx.ai/docs/integrations/opencode" },
  { label: "Hermes Agent", href: "https://flickerx.ai/docs/integrations/hermes-agent" },
];
...
const KEY_PLACEHOLDER = "sk-flickerx-...R_KEY";
const USE_TUNNEL_KEY = "flickerx_api_use_tunnel";
...
function useExampleModelName(): string | null {
  ...
  // Idle-unload on its own (UNFLICKERX_MODEL_IDLE_TTL, switching off) reloads exactly
...
const SHIKI_THEMES = [flickerxLightTheme, flickerxDarkTheme] as [
  typeof flickerxLightTheme,
  typeof flickerxDarkTheme,
];
...
export function UsageExamples({ apiKey }: { apiKey?: string | null }) {
  ...
  useEffect(() => {
    ...
    if (!localAgentDetection) {
      ...
      // FlickerX backend's PATH, which is meaningless now that this panel no
  ...
  // direction. `codex` needs a GGUF model (flickerx_cli's
  ...
  useEffect(() => {
    ...
    if (preferred) {
      ...
      // of leaving a codex command flickerx_cli will reject.
      setAgent(DEFAULT_AGENT);
    }
