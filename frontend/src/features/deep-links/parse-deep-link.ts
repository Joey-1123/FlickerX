...
export interface FlickerxDeepLinkIntent {
  model: string;
  file?: string;
}
...
export function parseFlickerxDeepLink(
  rawUrl: string,
): FlickerxDeepLinkIntent | null {
  const queryIndex = rawUrl.indexOf("?");
  const target = queryIndex === -1 ? rawUrl : rawUrl.slice(0, queryIndex);
  if (
    target !== "flickerx://open_from_hf" &&
    target !== "flickerx://open_from_hf/"
  ) {
    return null;
  }
