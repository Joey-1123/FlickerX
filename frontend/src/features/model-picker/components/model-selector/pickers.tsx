...
function isFlickerXPublisherRepoId(repoId: string): boolean {
  return isFlickerXOwner(splitRepoLabel(repoId).owner);
}
...
function ModelRow({
  ...
  /** Hide the "owner/" prefix (e.g. Recommended, where all are from the same publisher). */
  hideOwner?: boolean;
  ...
  // Drop our own owner: the list is nearly all from the same publisher/, so it is noise.
  // Other owners still show, which is what tells the two apart.
  const showOwner = !!owner && !hideOwner && !isFlickerXOwner(owner);
...
// Recommended section sort. "recommended" = newly created publisher GGUF/MLX that
// fit the device; the rest are plain HF sort keys over all flickerx models.
...
// Format filter dropdown for the FlickerX listing. Plain labels are reused in
...
export function HubModelPicker({
  ...
  catalog?: CatalogGroup[];
  /** Also surface community models carrying `task`'s pipeline tags, below
   *  the flickerx rows and in search. Opt-in, since the runtime has to load an arbitrary
  ...
  // and Hub run one implementation. Scoped to flickerx like the old listing.
  const online = useOnlineStatus();
  // Sanitize to anonymous on a malformed token, matching the Hub page.
  const accessToken = hfApiToken(hfToken);
  // Recommended section: a live flickerx listing sorted by the dropdown. The
  ...
  // "recommended" surfaces the most recently created FlickerX repos.
  ...
  // Two hooks for the same reason the flickerx pair exists: browse must not refetch
  // per keystroke, search must not be pinned to the empty query. pinFlickerXFirst is
  // off since the flickerx rows already sit above these.
  ...
  // The Fine-tuned section header; the train icon on the FlickerX header scrolls
  ...
  // The Other models header; the directions icon on the FlickerX header scrolls
  // here.
  ...
  // The Custom Folders header; the folder icon on the FlickerX header scrolls
  ...
  // Drop models FlickerX cannot run for chat. A task-scoped picker wants exactly the tasks the chat classifier calls unsupported, so it gates on the task.
  ...
  // Format filter toggle for the FlickerX listing.
  ...
  const catalogSeedRows = useMemo<HfModelResult[]>(() => {
    ...
    return dedupe(models.map((model) => model.id))
      ...
      .map((id) => ({
        ...
        // and non-flickerx ids never get a listing row to correct it.
  ...
  const recommendedRows = useMemo(() => {
    ...
    const flickerxRows = orderRecommendedRows({
    ...
    return [...flickerxRows, ...communityRows];
  }, [
    recommendedSearch.results,
    catalogSeedRows,
    downloadedSet,
  ...
  // Cached non-GGUF repos. In chat, passesTaskGate drops diffusers image repos; the Images picker keeps them, but only flickerx-hosted ones this backend can load. Base repos are cached as dependencies and fail the trust gate.
      ...
      sortCachedRepos(
        cachedModels.filter(
          (c) =>
            ...
            // Diffusion pickers: flickerx repos plus any repo the backend can LOAD. Gate on a curated ARTIFACT (what loadSpecFor resolves), not a group-key match: a base / uncurated-quant sibling matches the group by key but dead-ends at the trust gate.
            // A flickerx repo must also be a full pipeline: the fall-through loads uncataloged rows as "pipeline", and from_pretrained on a single-file checkpoint repo fails. Curated single-file artifacts stay, since loadSpecFor carries their filename.
            (!task ||
              (isFlickerxRepoId(c.repo_id) && !c.single_file) ||
  ...
  // Local ./models entries. Chat-only FlickerX runs GGUF (any host) and MLX (Mac only), so raw checkpoints there are hidden (mirrors the cached
    ...
    (id: string, meta: ModelSelectorChangeMeta) => {
      if (!task) {
        ...
        if (
          ...
          toast.error(
            `${id} is not a speech model FlickerX can run yet. The Audio page lists the families it supports.`,
            { duration: 7000 },
          );
          return;
        }
  ...
  const flickerxAdditionalOnDeviceModels = useMemo(
    () =>
      visibleAdditionalOnDeviceModels.filter((model) =>
        isFlickerXPublisherRepoId(model.id),
      ),
  ...
  const otherAdditionalOnDeviceModels = useMemo(
    () =>
      visibleAdditionalOnDeviceModels.filter(
        (model) => !isFlickerXPublisherRepoId(model.id),
      ),
    [visibleAdditionalOnDeviceModels],
  );
  ...
  // Pinned entries surface in their own section above the FlickerX heading.
  // GGUF quants pin individually and their repo stays listed below; non-GGUF
  // repos pin whole and leave the FlickerX / Other models groups.
  ...
  // Split downloaded models so non-FlickerX repos get their own "Other models"
  // section above Fine-tuned.
  const flickerxCachedGguf = useMemo(
    () => visibleCachedGguf.filter((c) => isFlickerXPublisherRepoId(c.repo_id)),
    [visibleCachedGguf],
  );
  ...
  const flickerxCachedModelRows = useMemo(
    () =>
      visibleCachedModelRows.filter(
        (c) =>
          isFlickerXPublisherRepoId(c.repo_id) &&
  ...
  // Param counts come straight off the flickerx listings the picker already
  ...
  const hfIds = useMemo(() => {
    // Only the FlickerX tab searches the HF listing.
    if (!showHfSection || section !== "recommended") return [];
    return searchIdsFrom(results, isFlickerXOwned);
  }, [results, showHfSection, section, searchIdsFrom, isFlickerXOwned]);

  // Community search hits, listed after the flickerx ones. Deduped against them so
  // a repo the flickerx listing already returned is not shown twice.
  ...
  /** FlickerX first, then community: one list so rows, keyboard order and the
  ...
  const hubOptionKeys = useMemo(() => {
    ...
    // Pinned rows sit above the FlickerX heading on the On Device tab.
    if (
      section === "downloaded" &&
      cachedReady &&
      !pinnedCollapsed &&
    ...
    // Downloaded (FlickerX) rows (query-filtered) on the On Device tab only.
    if (
      section === "downloaded" &&
      (cachedReady || flickerxAdditionalOnDeviceModels.length > 0) &&
      !downloadedCollapsed &&
      (flickerxCachedGguf.length > 0 ||
        flickerxCachedModelRows.length > 0 ||
        flickerxAdditionalOnDeviceModels.length > 0)
    ) {
      keys.push(
        ...flickerxCachedGguf.map((model) =>
          makeModelOptionKey("downloaded-gguf", model.repo_id),
        ),
      );
      keys.push(
        ...flickerxCachedModelRows.map((model) =>
          makeModelOptionKey("downloaded-model", model.repo_id),
        ),
      );
      keys.push(
        ...flickerxAdditionalOnDeviceModels.map((model) =>
          makeModelOptionKey("additional-on-device", model.id),
        ),
      );
    }

    // FlickerX-tab search keys (curated matches + HF flickerx results).
    ...
    // Other (non-FlickerX) downloaded rows sit just above Fine-tuned.
    if (
      section === "downloaded" &&
    ...
    localDirCollapsed,
    flickerxCachedGguf,
    flickerxCachedModelRows,
    flickerxAdditionalOnDeviceModels,
    otherCachedGguf,
  ...
  // On Device owns the downloaded and custom-folder models; the FlickerX tab
  ...
  // only applies to the FlickerX listing.
  const sectionSortDropdown =
    section === "recommended" ? (
      <HubOptionMenu
        ...
        ariaLabel="Sort FlickerX models"
        align="end"
  ...
  // The Other models section and its shortcut only show with non-FlickerX downloads.
  ...
  // Shared row renderers so Downloaded (FlickerX) and Other models render alike.
          ...
          >
            {showConnected ? (
              ...
              when custom folders below still have matches. */}
                ...
                {/* Pinned quants and models sit above the FlickerX heading so
              ...
              sections below. */}
                ...
                {/* Downloaded (FlickerX) stays visible (filtered) while searching. */}
                {showDownloaded &&
                (flickerxCachedGguf.length > 0 ||
                  flickerxCachedModelRows.length > 0 ||
                  flickerxAdditionalOnDeviceModels.length > 0) ? (
                  <>
                    ...
                    >
                      {/* Rows drop the flickerx/ prefix; the heading carries
                    it for the group. */}
                      FlickerX
                    </ListLabel>
                    {!downloadedCollapsed &&
                      flickerxCachedGguf.map(renderDownloadedGgufRow)}
                    {!downloadedCollapsed &&
                      flickerxCachedModelRows.map(renderDownloadedModelRow)}
                    {!downloadedCollapsed &&
                      flickerxAdditionalOnDeviceModels.map(
                        renderAdditionalOnDeviceModelRow,
                      )}
                  </>
                ) : null}

                {/* Other models: non-FlickerX downloads, grouped just above
                        ...
                        return (
                          <div key={id}>
                            <ModelRow
                              ...
                              // flickerx upload, and two publishers would collide.
                              hideOwner={isFlickerXOwned(id)}
