...
function VramBudgetRow() {
  ...
  // Stored beats UNFLICKERX_VRAM_FRACTION, so once the slider has been touched there
      ...
      <AdvancedGpuSlider
        ...
        info={
          <div className="flex flex-col gap-1.5">
            <div>
              Share of each GPU FlickerX will claim when it sizes the model and
            ...
            <div>
              Reset clears the stored value, so UNFLICKERX_VRAM_FRACTION applies
              again if it is set.
            </div>
          </div>
        }
      ...
      <div className={isDiffusion ? "hidden" : ROW_CLASS}>
        <div className="flex min-w-0 items-center gap-1.5">
          ...
          <InfoHint>
            <div className="flex flex-col gap-1.5">
              <div>
                <span className="font-medium">Default:</span> FlickerX fits the
      ...
      {showGpuPicker && (
        <div className="space-y-2">
          <div className="flex min-w-0 items-center gap-1.5">
            ...
            <InfoHint>
              By default, FlickerX chooses GPUs automatically. Editing this list
 ...
 *
 * llama-server documents 283 flags and FlickerX already emits or manages about 115
 * of them, so the long tail is a text box rather than 168 more controls. The
 * boundary is `validate_extra_args` on the backend, which refuses the flags FlickerX
 * owns; this row is the same judgement shown early, plus a check against the flags
 * THIS build documents, which a list shipped with FlickerX could not do.
 */
        ...
        <InfoHint>
          <div className="flex flex-col gap-1.5">
            ...
            <div>
              ...
              characters. Flags FlickerX owns, like the model, the port and the
              API key, are refused.
            </div>
          </div>
        </InfoHint>
...
export function ModelConfigPage({
  ...
  useEffect(() => {
    ...
    // FlickerX refuses, and that route answers without running `llama-server --help`.
          ...
          <>
            <div className="space-y-3">
              ...
              <p className="text-ui-11 leading-relaxed text-muted-foreground">
                FlickerX automatically fits the context to your device, using the
