...
export type LlamaFlagCatalog = {
  ...
  /** Flags FlickerX manages; the load refuses these outright. */
...
export function invalidateLlamaFlagCatalog(): void {
  ...
  // The managed answer too. Its denylist is FlickerX's own and no binary changes it,
...
/**
 * Just the flags FlickerX refuses, without the `--help` probe behind the catalogue.
 *
...
export function loadManagedLlamaFlags(): Promise<LlamaManagedFlags | null> {
  ...
  // exactly as the full catalogue does. The denylist in this answer is FlickerX's own
