...
/**
 * Resident, not merely picked. Loading an image or video model evicts the chat
 * model (the GPU arbiter allows a single GPU owner) and leaves the selection
 * alone, so a check on the selection reported a model the backend had already
 * released and the next prompt failed with a bare 400.
 *
 * `undefined` means the first status read has not landed: assume loaded, or the
 * header flashes "not loaded" on every startup.
 */
