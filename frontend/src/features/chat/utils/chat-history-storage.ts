...
const LEGACY_CHAT_IMPORT_KEY = "flickerx_chat_legacy_imported_to_studio_db";
...
export function isExpectedBackgroundChatStorageError(error: unknown): boolean {
  return (
    ...
    (error.message === "Invalid or expired token" ||
      ...
      error.message === "FlickerX isn't running -- please relaunch it.")
  );
}
...
// Fast-path: check whether the "flickerx-chat" DB exists without opening
...
async function importLegacyChatsIfNeeded(): Promise<void> {
  ...
  legacyChatImportPromise = (async () => {
    // Fast-path: no Dexie DB -- new user, never had browser-only FlickerX.
