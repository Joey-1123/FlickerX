...
/**
 * Items already sent are fixed, so nothing may move to or from a slot before
 * the one about to dispatch.
 */
...
/**
 * The queue after the move, or null when the move is not allowed. `toIndex` is
 * read off the original array, so a downward drag lands after the target and an
 * upward drag lands before it, which is how a dragged row reads on screen.
 */
...
/**
 * Whether the move changed which item dispatches next, so the caller knows to
 * retarget the pending send.
 */
