...
/**
 * Query for GET /api/models/gguf-variants. A Hub already known to be unreachable is asked
 * for the cached answer instead of a remote listing that cannot arrive, as the Hub client does.
 */
...
/**
 * Signal every variant request carries: the caller's abort (the expander drops the request
 * when its row collapses) folded with the timeout. Callers MUST dispose once settled.
 */
