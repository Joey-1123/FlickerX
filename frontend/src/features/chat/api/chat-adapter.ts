...
async function ensureDefaultModelDownloaded(
  ...
  const description =
    `FlickerX couldn’t find an existing model. FlickerX is now getting ` +
...
async function autoLoadSmallestModel(options?: AutoLoadOptions): Promise<{
  ...
  function noteLoadFailure(label: string, error: unknown): void {
    ...
    const marker = error as { flickerxTransportFailure?: boolean; flickerxUserCancelled?: boolean };
    const blamesModel = !(
      marker?.flickerxTransportFailure === true || marker?.flickerxUserCancelled === true
    );
    loadFailure.current = {
      label,
  ...
  function recordCandidateFailure(label: string, error: unknown): void {
    ...
    const marker = error as { flickerxUserCancelled?: boolean };
    noteLoadFailure(label, error);
    if (marker?.flickerxUserCancelled === true) {
      ...
      try {
        ...
        const buildRequestPayload = async (
          ...
          return {
            ...
            // (flickerx run --enable-tools) can open the tool loop with no pill,
