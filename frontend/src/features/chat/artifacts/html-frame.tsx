...
// offer their own UI, but downloads must go through FlickerX's explicit
...
export function ArtifactHtmlFrame({
  ...
  const postArtifactHtml = useCallback(() => {
    ...
    iframeRef.current?.contentWindow?.postMessage(
      { type: "flickerx:artifact-html", html: artifactHtml },
      "*",
    );
  }, [artifactHtml]);
  ...
  useEffect(() => {
    const handler = (event: MessageEvent) => {
      ...
      if (event.data?.type === "flickerx:artifact-blocked") {
