  ...
  if (kind === WORKING_DIRECTORY_UNAVAILABLE) {
    const cause =
      typeof navigator !== "undefined" && /Win/i.test(navigator.platform ?? "")
        ...
        : "";
    return `FlickerX cannot reach your user folder, so it has nowhere to run from.${cause} Reconnect and try again.`;
  }
  // Also not an install problem, and not the folder either: one of FlickerX's own
  // path settings names somewhere unresolvable, so the value is the fix.
  if (kind === PATH_SETTING_UNRESOLVABLE) {
    const which = setting ? `${setting} points` : "One of FlickerX's folder settings points";
    return `${which} somewhere that cannot be resolved, so FlickerX has nowhere safe to run from. Set it to a full path, such as D:\\flickerx-cache, and try again.`;
  }
  if (disposition === "owned_stale") {
    return "Desktop-owned FlickerX backend is too old for this desktop app. Run `flickerx update`, then restart FlickerX.";
  }
  return "Managed FlickerX install is too old. Run `flickerx update`.";
}
